...
#include <all_estimator/Acc_2D_to_3D_insnum.hpp>
...
bool YoloBridge::bridge(denso_srvs::Acc_bridge::Request &req, denso_srvs::Acc_bridge::Response &res){
    // get_one_message<denso_msgs::yolo_bridge>(req, YOLO_topic_name_, nh_, timespan_);
    ROS_INFO_STREAM("iijanai" << req.x.size() << ":" << req.y.size() << ";" << req.z.size());
    // trans_cloud_.clear();
    float x_min = 0;
    float x_max = 0;
    float y_min = 0;
    ...
    float z_min = 0;
    float z_max = 0;
    trans_cloud_.resize(req.x.size());
    for (int i=0; i<req.x.size(); i++){
        trans_cloud_.points[i].x = req.x[i];
        trans_cloud_.points[i].y = req.y[i];
        trans_cloud_.points[i].z = req.z[i];

        if (trans_cloud_.points[i].x > x_max){
            x_max = trans_cloud_.points[i].x;
        } 
        if (trans_cloud_.points[i].x < x_min){
            x_min = trans_cloud_.points[i].x;
        }
        if (trans_cloud_.points[i].y > y_max){
            y_max = trans_cloud_.points[i].y;
        } 
        if (trans_cloud_.points[i].y < y_min){
            y_min = trans_cloud_.points[i].y;
        }
        if (trans_cloud_.points[i].z > z_max){
            z_max = trans_cloud_.points[i].z;
        } 
        if (trans_cloud_.points[i].z < z_min){
            z_min = trans_cloud_.points[i].z;
        }
        
    }
    ROS_INFO_STREAM("x_max;" << x_max << " x_min;" << x_min << " y_max;" << y_max << " y_min;" << y_min << " z_max;" << z_max << " z_min;" << z_min);
    // trans_cloud_.header.frame_id = "world";
    // ROS_INFO_STREAM("trans" << trans_cloud_.size());
    // get_one_message(cloud_msgs_, inputcloud_topic_name_, nh_, timespan_);
    // cloud_msgs_ = req.in_cloud;
    ...
    // output_cloud_msgs_.header.frame_id = "world";
    // output_cloud_msgs_.header.frame_id = "photoneo_center_optical_frame";
    output_cloud_msgs_.header.frame_id = frame_id_;
    ...
    ROS_INFO_STREAM(res.x[0].data.size());
    ROS_INFO_STREAM(res.y[0].data.size());
    ROS_INFO_STREAM(res.z[0].data.size());
...
std::vector<std::vector<int>> YoloBridge::write_instance(std::vector<std::vector<cv::Point2d>> point_2d, cv::Mat img_size_img, std::vector<int>& pcl_num)
{
    ...
    ROS_INFO_STREAM("instance_img_sasyo" << instance_img_sasyo[0].size());
    ...
    for (pcl::PointCloud<pcl::PointXYZ>::iterator pt = input_pcl_cloud.points.begin(); pt != input_pcl_cloud.points.end(); ++pt)
    {
       ...
       // cv::Mat rgb_image;
            ...
            && uv.y < (rgb_image.rows / scale))
        {
            ...
            // ROS_INFO_STREAM("na~ru" << instance[y][x] << "hodo");
            ...
            // cv::circle(cv_image, cv::Point(uv.x, uv.y), 10, cv::Scalar(255, 0, 0), 1, 1);
        
            for(int j = 0; j < Bbox_num_; j++){
                // ROS_INFO_STREAM("na~ru" << instance[y][x] << "hodo" << j+1);
                if (instance[y][x] == j+1) { //ここの数がどのバウンディングボックスをパブリッシュするか決定
                    // ROS_INFO_STREAM("kokodaro");
            ...
            // //         ROS_INFO_STREAM("kokonanokamosikasuruto");
                    break;
                }
            // }
            // // else {
...
cv::Point2d YoloBridge::project3d_to_pixel_origin(cv::Point3d xyz, sensor_msgs::CameraInfo cinfo_msg)
{
    ...
    // ROS_INFO_STREAM(cloud_msgs_.K[0]);
    return uv_rect;
}
