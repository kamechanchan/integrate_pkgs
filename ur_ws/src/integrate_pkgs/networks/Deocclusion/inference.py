...
def to_eraser(inst, bbox, newbbox):
    ...
    offbbox = [newbbox[0] - bbox[0], newbbox[1] -
               bbox[1], newbbox[2], newbbox[3]]
...
def net_forward(model, image, inmodal_patch, eraser, use_rgb, th):
    if use_rgb:
        image = torch.from_numpy(image.transpose(
            (2, 0, 1)).astype(np.float32)).unsqueeze(0)
        image = image.cuda()
    inmodal_patch = torch.from_numpy(inmodal_patch.astype(
        np.float32)).unsqueeze(0).unsqueeze(0).cuda()
    with torch.no_grad():
        if eraser is not None:
            eraser = torch.from_numpy(eraser.astype(
                np.float32)).unsqueeze(0).unsqueeze(0).cuda()
            if use_rgb:
                output = model.model(
                    torch.cat([inmodal_patch, eraser], dim=1), image)
            else:
...
def net_forward_ordernet(model, image, inmodal1, inmodal2, use_rgb):
    if use_rgb:
        image = torch.from_numpy(image.transpose(
            (2, 0, 1)).astype(np.float32)).unsqueeze(0).cuda()
...
def infer_amodal_hull(inmodal, bboxes, order_matrix, order_grounded=True):
    ...
    for i in range(num):
        ...
        if order_grounded:
            ...
            eraser = (inmodal[ancestors, ...].sum(
                axis=0) > 0).astype(np.uint8)  # union
            hull[(eraser == 0) & (m == 0)] = 0
        amodal.append(hull)
    return amodal
        ...
        for j in range(i + 1, num):
            if bordering(inmodal[i], inmodal[j]):
                ...
                occ_value_matrix[i, j] = (
                    (amodal_i > inmodal[i]) & (inmodal[j] == 1)).sum()
                occ_value_matrix[j, i] = (
                    (amodal_j > inmodal[j]) & (inmodal[i] == 1)).sum()
...
def infer_order_yaxis(inmodal):
    ...
    for i in range(num):
        for j in range(i + 1, num):
            if bordering(inmodal[i], inmodal[j]):
                center_i = [coord.mean()
                            for coord in np.where(inmodal[i] == 1)]  # y, x
                center_j = [coord.mean()
                            for coord in np.where(inmodal[j] == 1)]  # y, x
    ...
    for i in range(num):
        for j in range(i + 1, num):
            if bordering(inmodal[i], inmodal[j]):
                ...
                size = max([np.sqrt(bbox[2] * bbox[3] * 2.),
...
def infer_order(model, image, inmodal, bboxes, use_rgb=True, th=0.5, dilate_kernel=0, input_size=None, min_input_size=32, interp='nearest', debug_info=False):
    '''
    ...
    ind = []
    # print("kamisama")
    # print(inmodal[0].shape)
    # print(np.array(inmodal).shape)
    ...
    for i in range(num):
        for j in range(i + 1, num):
            ...
            if deal_with_fullcover:
                print("koko")
                fullcover = fullcovering(
                    inmodal[i], inmodal[j], bboxes[i], bboxes[j])
                if fullcover == 1:
                    fullcover_inds.append([i, j])
                elif fullcover == 2:
                    fullcover_inds.append([j, i])
    mitukeru_occ = time.time()
    print("mituketa"+str(mitukeru_occ-start))
    pairnum = len(ind)
    if pairnum == 0:
        print("mouwakatta")
        return order_matrix
    # print("iyanahitohaiya")
    # print(ind)
    ind = np.array(ind)
    eraser_patches = []
    inmodal_patches = []
    ...
    for i in range(pairnum):
        ...
        tid = ind[i, 0]
        eid = ind[i, 1]
        image_patch = utils.crop_padding(
            image, bboxes[tid].data, pad_value=(0, 0, 0))
        inmodal_patch = utils.crop_padding(
            inmodal[tid], bboxes[tid].data, pad_value=(0,))
        if input_size is not None:
            newsize = input_size
        # elif min_input_size > bboxes[tid, 2]:
        elif min_input_size > bboxes[tid].data[2]:
            newsize = min_input_size
        else:
            newsize = None
        if newsize is not None:
            inmodal_patch = resize_mask(inmodal_patch, newsize, interp)
        eraser = utils.crop_padding(inmodal[eid], bboxes[tid].data, pad_value=(0,))
        if newsize is not None:
        ...
        amodal_patches.append(net_forward(
            model, image_patch, inmodal_patch, eraser, use_rgb, th))
        # ratios:リサイズ率
        # ratios.append(
        #     1. if newsize is None else bboxes[tid, 2] / float(newsize))
        ratios.append(
            1. if newsize is None else bboxes[tid].data[2] / float(newsize))
        end_loop = time.time()
        print("loop"+str(end_loop-start_loop))
        cnt += i
    print(i)
    syori_main = time.time()
    print("main"+str(syori_main-mitukeru_occ))

    occ_value_matrix = np.zeros((num, num), dtype=np.float32)
    # print("network:output")
    # print(np.array(amodal_patches).shape)
...
def bordering(a, b):
    ...
    # print("np")
    # # print((a_dilate == 1) & b)
    # print(np.any((a_dilate == 1) & b))
    # cnt_a = 0
    # cnt_b = 0
    # cnt_c = 0
    # for i in range(a_dilate.shape[0]):
    #     for j in range(a_dilate.shape[1]):
    #         if a_dilate[i][j] == 1 and b[i][j]:
    #             cnt_a += 1
    #         # elif a_dilate[i][j] == 1:
    #         #     cnt_b += 1
    #         else :
    ...
    # print(a_dilate.shape)
    # print(a_dilate.dtype)
    # print(b.shape)
    # print(b.dtype)
...
def eval_order(order_matrix, gt_order_matrix):
    ...
    occpair_true = ((order_matrix == gt_order_matrix)
                    & (gt_order_matrix != 0)).sum() / 2
    ...
    show_err = np.concatenate(
        [np.array(err).T + 1, gt_err[:, np.newaxis], pred_err[:, np.newaxis]], axis=1)
    ...
    for i in range(num):
        ...
        if rgb is not None:
            ...
            rgb_patch = cv2.resize(utils.crop_padding(rgb, new_bboxes[i], pad_value=(0, 0, 0)),
                                   (input_size, input_size), interpolation=cv2.INTER_CUBIC)
            prob_crf = np.array(utils.densecrf(
                prob, rgb_patch)).reshape(*prob.shape)
            pred = (prob_crf[1, :, :] > th).astype(np.uint8)  # HW
        else:
            pred = (output[0, 1, :, :] > th).cpu(
            ).numpy().astype(np.uint8)  # HW
